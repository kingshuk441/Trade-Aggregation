...
public class CancelServiceTest {
        ...
        Trade TRADE_1 = new Trade(1, "#T1", "PNBR", "PNB RAIPUR", "SBITN", "SBI TAMIL NADU", LocalDate.now(),
                        LocalDate.now(), "BOND", 5000, LocalDate.now(), "INR", "SELLER", "BUYER", new Date(),
                        new Date(),
                        new Date(), 1, "UF", 10);
        ...
        public void save_AggrTrade_success() {
                ...
                Trade newTrade1 = new Trade(5, "#T5", "PNBR", "PNB RAIPUR", "SBITN", "SBI TAMIL NADU", LocalDate.now(),
                                LocalDate.now(), "BOND", 3000, LocalDate.now(), "DLR", "SELLER", "BUYER", new Date(),
                                new Date(),
                                new Date(), 0, "UF", 10);

                Trade aggregatedTrade = new Trade(6, "#T6", "PNBR", "PNB RAIPUR", "SBITN", "SBI TAMIL NADU",
                                LocalDate.now(),
                                LocalDate.now(), "BOND", 8000, LocalDate.now(), "DLR", "SELLER", "BUYER", new Date(),
                                new Date(),
                                new Date(), 1, "AGG", 10);
                ...
                TradeBody tradeBody = new TradeBody("#T5", "PNBR", "SBITN", LocalDate.now(), LocalDate.now(), "BOND",
                                5000,
                ...
                boolean res = trade.get(0).getAggregatedFrom().size() == 2
                                && trade.get(0).getAggregatedFrom().get(0).getTradeRefNum().equals(
                                                "#T1")
                                && trade.get(0).getAggregatedFrom().get(1).getTradeRefNum().equals(
                                                "#T5");
